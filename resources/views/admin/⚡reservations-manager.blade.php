...
<div>
    <flux:main class="space-y-6">
        ...
        {{-- Header with Stats --}}
        <flux:container class="bg-linear-to-r from-blue-600 to-purple-600 text-white">
            <flux:container.inner class="py-8">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row">
                        <flux:button wire:click="openModal" variant="primary" icon="plus" class="bg-white text-blue-600 hover:bg-blue-50">
                            New Reservation
                        </flux:button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
                    <flux:card class="bg-white/10 backdrop-blur border-white/20">
                        <flux:card.content class="p-4 text-center">
                    ...
                    <flux:card class="bg-white/10 backdrop-blur border-white/20">
                        <flux:card.content class="p-4 text-center">
                            <flux:heading size="lg" class="text-white">{{ $this->getTotalGuestsToday() }}</flux:heading>
            ...
            <flux:container.inner class="p-6">
                ...
                <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <flux:field>
                        ...
                        <flux:select 
                            wire:model.live="filterPartySize" 
                            variant="listbox" 
        ...
        <flux:container>
            <flux:container.inner class="p-6">
                ...
                <flux:table :paginate="$this->reservations">
                    <flux:table.head>
                        <flux:table.row>
                            <flux:table.header>Guest Details</flux:table.header>
                            <flux:table.header>Reservation</flux:table.header>
                            <flux:table.header>Party</flux:table.header>
                            <flux:table.header>Status</flux:table.header>
                            <flux:table.header>Actions</flux:table.header>
                        </flux:table.row>
                    </flux:table.head>
                        ...
                        <flux:table.row wire:key="res-{{ $reservation->id }}">
                            <flux:table.cell>
                                <div class="space-y-1">
                                    ...
                                    <p class="text-xs text-zinc-400 italic mt-1">{{ Str::limit($reservation->notes, 50) }}</p>
                                    @endif
                                </div>
                            </flux:table.cell>
                            ...
                            <flux:table.cell>
                                <div class="space-y-1">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="font-medium">{{ $reservation->reservation_date->format('M j, Y') }}</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <flux:icon name="clock" class="size-4 text-zinc-400" />
                                        <span class="text-sm">{{ \Carbon\Carbon::parse($reservation->reservation_time)->format('g:i A') }}</span>
                                    </div>
                                </div>
                            </flux:table.cell>
                            ...
                            <flux:table.cell>
                                ...
                                >
                                    @foreach($this->statuses() as $s)
                                    <flux:select.option value="{{ $s->value }}" :selected="$reservation->status === $s">
                                        {{ $s->label() }}
                                    </flux:select.option>
                                    @endforeach
                                </flux:select>
                            ...
                            <flux:table.cell>
                                <flux:dropdown>
                                    ...
                                    <flux:dropdown.content align="end">
                                        <flux:dropdown.item 
                                            wire:click="openModal({{ $reservation->id }})"
                                            icon="pencil"
                                        >
                                            Edit Reservation
                                        </flux:dropdown.item>
                        ...
                        <flux:table.row>
                            <flux:table.cell colspan="5" class="py-12">
                                <div class="text-center">
                                    ...
                                    <flux:heading size="md" class="text-zinc-600">No reservations found</flux:heading>
    ...
    <flux:modal wire:model="showModal" class="md:max-w-2xl">
        <flux:modal.content class="space-y-6">
            <flux:modal.header>
                    ...
                    <flux:fieldset>
                        ...
                        <flux:field>
                            ...
                            <flux:input 
                                ...
                                type="number" 
                                min="1" 
                                max="{{ App\Services\ReservationService::getMaxPartySize() }}" 
                                icon="users"
                            />
                        </flux:field>
